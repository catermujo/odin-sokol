...
when ODIN_OS == .Windows {
    ...
    when USE_DLL {
        when USE_GL && ODIN_ARCH ==
            .arm64 && DEBUG { foreign import sokol_time_clib "sokol_dylib_macos_arm64_gl_debug.dylib" } else when USE_GL && ODIN_ARCH == .arm64 && !DEBUG { foreign import sokol_time_clib "sokol_dylib_macos_arm64_gl_release.dylib" } else when USE_GL && ODIN_ARCH == .amd64 && DEBUG { foreign import sokol_time_clib "sokol_dylib_macos_x64_gl_debug.dylib" } else when USE_GL && ODIN_ARCH == .amd64 && !DEBUG { foreign import sokol_time_clib "sokol_dylib_macos_x64_gl_release.dylib" } else when !USE_GL && ODIN_ARCH == .arm64 && DEBUG { foreign import sokol_time_clib "sokol_dylib_macos_arm64_metal_debug.dylib" } else when !USE_GL && ODIN_ARCH == .arm64 && !DEBUG { foreign import sokol_time_clib "sokol_dylib_macos_arm64_metal_release.dylib" } else when !USE_GL && ODIN_ARCH == .amd64 && DEBUG { foreign import sokol_time_clib "sokol_dylib_macos_x64_metal_debug.dylib" } else when !USE_GL && ODIN_ARCH == .amd64 && !DEBUG { foreign import sokol_time_clib "sokol_dylib_macos_x64_metal_release.dylib" }
    } else {
        when USE_GL {
            when ODIN_ARCH == .arm64 {
                when DEBUG { foreign import sokol_time_clib "sokol_time_macos_arm64_gl_debug.a" } else { foreign import sokol_time_clib "sokol_time_macos_arm64_gl_release.a" }
            } else {
                when DEBUG { foreign import sokol_time_clib "sokol_time_macos_x64_gl_debug.a" } else { foreign import sokol_time_clib "sokol_time_macos_x64_gl_release.a" }
            }
        } else {
            when ODIN_ARCH == .arm64 {
                when DEBUG { foreign import sokol_time_clib "sokol_time_macos_arm64_metal_debug.a" } else { foreign import sokol_time_clib "sokol_time_macos_arm64_metal_release.a" }
            } else {
                when DEBUG { foreign import sokol_time_clib "sokol_time_macos_x64_metal_debug.a" } else { foreign import sokol_time_clib "sokol_time_macos_x64_metal_release.a" }
            }
        }
    }
