...
init :: proc "c" () {
    ...
    state.compute.pip = sg.make_pipeline(
        {compute = true, shader = sg.make_shader(update_shader_desc(sg.query_backend())), label = "compute-pipeline"},
    )
    ...
    state.display.bind.vertex_buffers[0] = sg.make_buffer(
        {data = {ptr = &vertices, size = size_of(vertices)}, label = "geometry-vbuf"},
    )
    indices := [?]u16{0, 1, 2, 0, 2, 3, 0, 3, 4, 0, 4, 1, 5, 1, 2, 5, 2, 3, 5, 3, 4, 5, 4, 1}
    state.display.bind.index_buffer = sg.make_buffer(
        {usage = {index_buffer = true}, data = {ptr = &indices, size = size_of(indices)}, label = "geometry-ibuf"},
    )
...
frame :: proc "c" () {
    ...
    sg.begin_pass({action = state.display.pass_action, swapchain = sglue.swapchain(), label = "render-pass"})
...
compute_vs_params :: proc(ry: f32) -> Vs_Params {
    ...
    return Vs_Params{mvp = m.mul(view_proj, m.rotate(ry, {0.0, 1.0, 0.0}))}
}
