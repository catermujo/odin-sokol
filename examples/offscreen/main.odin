...
init :: proc "c" () {
    ...
    state.offscreen.pass.attachments.depth_stencil = sg.make_view({depth_stencil_attachment = {image = depth_img}})

    // the display render pass needs a texture view on the color image
    state.display.bind.views[VIEW_tex] = sg.make_view({texture = {image = color_img}})
