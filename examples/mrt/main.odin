...
recreate_offscreen_attachments :: proc(width, height: i32) {
    ...
    for i in 0 ..< NUM_MRTS {
        ...
        state.images.color[i] = sg.make_image(
            {usage = {color_attachment = true}, width = width, height = height, sample_count = OFFSCREEN_SAMPLE_COUNT},
        )
        sg.destroy_view(state.offscreen.pass.attachments.colors[i])
        state.offscreen.pass.attachments.colors[i] = sg.make_view({color_attachment = {image = state.images.color[i]}})

        // resolve image and views
        sg.destroy_image(state.images.resolve[i])
        state.images.resolve[i] = sg.make_image(
            {usage = {resolve_attachment = true}, width = width, height = height, sample_count = 1},
        )
        ...
        state.display.bind.views[i] = sg.make_view({texture = {image = state.images.resolve[i]}})
    }
...
init :: proc "c" () {
    ...
    state.offscreen.bind.index_buffer = sg.make_buffer(
        {usage = {index_buffer = true}, data = {ptr = &cube_indices, size = size_of(cube_indices)}},
    )
    ...
    quad_vbuf := sg.make_buffer({data = {ptr = &quad_vertices, size = size_of(quad_vertices)}})
