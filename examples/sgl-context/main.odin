...
init :: proc "c" () {
    ...
    sgl.setup({max_vertices = 64, max_commands = 16, logger = {func = slog.func}})

    // pass action and pipeline for the default render pass
    state.display.pass_action = {
        colors = {0 = {load_action = .CLEAR, clear_value = {0.5, 0.7, 1.0, 1.0}}},
    }
    state.display.sgl_pip = sgl.context_make_pipeline(
        sgl.default_context(),
        {cull_mode = .BACK, depth = {write_enabled = true, compare = .LESS_EQUAL}},
    )
