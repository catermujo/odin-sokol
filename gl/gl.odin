...
when ODIN_OS == .Windows {
    when ODIN_ARCH == .arm64 {
        when USE_DLL {
            when USE_GL {
                when DEBUG { foreign import sokol_gl_clib "../sokol_dll_windows_arm64_gl_debug.lib" } else { foreign import sokol_gl_clib "../sokol_dll_windows_arm64_gl_release.lib" }
            } else {
                when DEBUG { foreign import sokol_gl_clib "../sokol_dll_windows_arm64_d3d11_debug.lib" } else { foreign import sokol_gl_clib "../sokol_dll_windows_arm64_d3d11_release.lib" }
            }
        } else {
            when USE_GL {
                when DEBUG { foreign import sokol_gl_clib "sokol_gl_windows_arm64_gl_debug.lib" } else { foreign import sokol_gl_clib "sokol_gl_windows_arm64_gl_release.lib" }
            } else {
                when DEBUG { foreign import sokol_gl_clib "sokol_gl_windows_arm64_d3d11_debug.lib" } else { foreign import sokol_gl_clib "sokol_gl_windows_arm64_d3d11_release.lib" }
            }
        }
    } else when ODIN_ARCH == .amd64 {
        when USE_DLL {
            when USE_GL {
                when DEBUG { foreign import sokol_gl_clib "../sokol_dll_windows_x64_gl_debug.lib" } else { foreign import sokol_gl_clib "../sokol_dll_windows_x64_gl_release.lib" }
            } else {
                when DEBUG { foreign import sokol_gl_clib "../sokol_dll_windows_x64_d3d11_debug.lib" } else { foreign import sokol_gl_clib "../sokol_dll_windows_x64_d3d11_release.lib" }
            }
        } else {
            when USE_GL {
                when DEBUG { foreign import sokol_gl_clib "sokol_gl_windows_x64_gl_debug.lib" } else { foreign import sokol_gl_clib "sokol_gl_windows_x64_gl_release.lib" }
            } else {
                when DEBUG { foreign import sokol_gl_clib "sokol_gl_windows_x64_d3d11_debug.lib" } else { foreign import sokol_gl_clib "sokol_gl_windows_x64_d3d11_release.lib" }
            }
        }
    } else {
    ...
    when ODIN_ARCH == .amd64 {
        when USE_DLL {
            when DEBUG { foreign import sokol_gl_clib "sokol_gl_linux_x64_gl_debug.so" } else { foreign import sokol_gl_clib "sokol_gl_linux_x64_gl_release.so" }
        } else {
            when DEBUG { foreign import sokol_gl_clib "sokol_gl_linux_x64_gl_debug.a" } else { foreign import sokol_gl_clib "sokol_gl_linux_x64_gl_release.a" }
        }
    } else when ODIN_ARCH == .arm64 {
        when USE_DLL {
            when DEBUG { foreign import sokol_gl_clib "sokol_gl_linux_arm64_gl_debug.so" } else { foreign import sokol_gl_clib "sokol_gl_linux_arm64_gl_release.so" }
        } else {
            when DEBUG { foreign import sokol_gl_clib "sokol_gl_linux_arm64_gl_debug.a" } else { foreign import sokol_gl_clib "sokol_gl_linux_arm64_gl_release.a" }
        }
    } else {
